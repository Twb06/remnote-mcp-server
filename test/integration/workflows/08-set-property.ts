 ...
 *
 * Validates remnote_set_property through the shared Automation Bridge Test Tag
 * fixture. The fixture must be a property-bearing tag/table with an
...
function makePropertyValue(runId: string): string {
  ...
  return `mcp-set-property-${runId}-${randomSuffix}`;
}
...
async function findVerifiedPropertyRow(
  ...
  do {
    const page = (await ctx.client.callTool('remnote_read_table', {
      tableRemId,
      propertyFilter: [propertyName],
      limit: READ_TABLE_PAGE_SIZE,
      offset,
...
export async function setPropertyWorkflow(
  ...
  // Step 1: Resolve the property-bearing test tag by exact title.
  {
    ...
    try {
      const result = (await ctx.client.callTool('remnote_search', {
        query: PROPERTY_FIXTURE_TAG_TITLE,
        limit: 150,
        contentMode: 'none',
  ...
  // Step 2: Resolve the automation-level property ID from the tag/table schema.
  {
    ...
    try {
      ...
      const table = (await ctx.client.callTool('remnote_read_table', {
  ...
  // Step 3: Set a unique text property value on the test note.
  {
    ...
    try {
      ...
      const result = (await ctx.client.callTool('remnote_set_property', {
        remId: targetRemId,
      ...
      assertEqual(result.remId, targetRemId, 'set_property remId');
      assertEqual(result.tagRemId, propertyFixtureTagRemId, 'set_property tagRemId');
      assertEqual(result.propertyRemId, automationLevelPropertyRemId, 'set_property propertyRemId');
      assertEqual(result.valueKind, 'text', 'set_property valueKind');
  ...
  // Step 4: Verify the value through the tag/table read surface and keep it for manual inspection.
  {
    ...
    try {
      ...
      steps.push({
        label: `Verify kept ${PROPERTY_FIXTURE_NAME} exact reference value via read_table`,
        passed: true,
        durationMs: Date.now() - start,
      });
    } catch (e) {
      steps.push({
        label: `Verify ${PROPERTY_FIXTURE_NAME} value via read_table`,
        passed: false,
        durationMs: Date.now() - start,
        error: (e as Error).message,
      });
