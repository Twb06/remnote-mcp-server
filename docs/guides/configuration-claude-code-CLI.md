...
> **Zero-config alternative:** If you want to use RemNote from Claude Code (or any other coding harness) without MCP
> client configuration, use the bundled `remnote-cli` command — just paste a single prompt that loads the skill file.
> See [Use RemNote from Any Coding Harness](../demo.md#use-remnote-from-any-coding-harness).
